...
- [x] 42 задачи со starter, statement, public/hidden tests и reference solution: 25 алгоритмических, 7 на конкурентность и 10 на ООП и ошибки.
...
- [x] Синхронизация позиции курсора и видимая каретка соперника в Monaco.
- [x] Локальный Go-autocomplete со snippets и подсказками стандартной библиотеки.
- [x] Статусы judge и панель результата в интерфейсе.
- [x] Фиксация победителя и счёта после победы, показ победного кода и проверки вне зачёта.
- [x] Ready-flow, общий счёт и бесконечная серия.
- [x] Пропуск задачи по голосу обоих игроков без начисления очков.
- [x] Одиночная тренировка: каталог задач, черновик кода и отметка «решено».
- [x] Задачи без повторов до исчерпания пула; граница циклов без повтора.
- [x] Матч не блокируется и не завершается при временном разрыве соединения.
